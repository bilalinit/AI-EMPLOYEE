#!/usr/bin/env python3
"""
Meta Cron Trigger - Scheduled Facebook/Instagram Post Generation

This script is designed to be run via cron to automatically trigger
the Meta posting skill at scheduled times.

Flow:
  Cron → This Script → Claude Code → /meta-posting skill
                                  ↓
              ...
              Orchestrator triggers execute-approved
                                  ↓
                      Post goes live on Facebook/Instagram

Usage:
    python meta_cron_trigger.py

Cron entry (daily at 9am):
    0 9 * * * cd /path/to/ai_employee_scripts && uv run python scripts/meta_cron_trigger.py
"""
...
CRON_LOG = VAULT_PATH / 'Logs' / 'meta_cron.log'
...
def setup_logging():
    """Configure logging to both console and meta_cron.log file."""
    # Ensure Logs directory exists
    CRON_LOG.parent.mkdir(parents=True, exist_ok=True)

    # Create logger
    logger = logging.getLogger('MetaCron')
    ...
    # File handler (append to meta_cron.log)
...
def call_claude_skill(skill_name: str, vault_path: Path, logger: logging.Logger, timeout: int = 1800) -> bool:
    ...
    Args:
        skill_name: Name of the skill to execute (e.g., 'meta-posting')
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("META CRON TRIGGER STARTED")
    logger.info("=" * 60)
    ...
    # Call the Meta posting skill
    logger.info("Triggering /meta-posting skill...")
    logger.info("")

    success = call_claude_skill(
        skill_name='meta-posting',
    ...
    if success:
        ...
        logger.info("  2. Add Instagram image URL if posting to Instagram")
        logger.info("  3. Select platforms (Facebook/Instagram/Both)")
        logger.info("  4. Review and edit if needed")
        logger.info("  5. Move to Approved/ when ready")
        logger.info("  6. Orchestrator will auto-post via execute-approved")
        logger.info("=" * 60)
        sys.exit(0)
    else:
        logger.error("CRON TRIGGER FAILED")
        logger.error("Check meta_cron.log for details")
        logger.error("=" * 60)
        sys.exit(1)
