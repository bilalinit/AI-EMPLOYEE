#!/usr/bin/env python3
"""
Twitter Cron Trigger - Scheduled Twitter/X Post Generation

This script is designed to be run via cron to automatically trigger
the Twitter posting skill at scheduled times.

Flow:
  Cron → This Script → Claude Code → /twitter-posting skill
                                  ↓
              ...
              Orchestrator triggers execute-approved
                                  ↓
                      Post goes live on Twitter/X

Usage:
    python twitter_cron_trigger.py

Cron entry (daily at 4am):
    0 4 * * * cd /path/to/ai_employee_scripts && uv run python scripts/twitter_cron_trigger.py
"""
...
CRON_LOG = VAULT_PATH / 'Logs' / 'twitter_cron.log'
...
def setup_logging():
    """Configure logging to both console and twitter_cron.log file."""
    # Ensure Logs directory exists
    CRON_LOG.parent.mkdir(parents=True, exist_ok=True)

    # Create logger
    logger = logging.getLogger('TwitterCron')
    ...
    # File handler (append to twitter_cron.log)
...
def call_claude_skill(skill_name: str, vault_path: Path, logger: logging.Logger, timeout: int = 1800) -> bool:
    ...
    Args:
        skill_name: Name of the skill to execute (e.g., 'twitter-posting')
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("TWITTER CRON TRIGGER STARTED")
    logger.info("=" * 60)
    ...
    # Call the Twitter posting skill
    logger.info("Triggering /twitter-posting skill...")
    logger.info("")

    success = call_claude_skill(
        skill_name='twitter-posting',
    ...
    if success:
        ...
        logger.error("Check twitter_cron.log for details")
        logger.error("=" * 60)
        sys.exit(1)
